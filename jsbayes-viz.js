(function (window) {
    ...
    return {
      ...
      addNode: function (id, svg_id, label, values, probs) {
        ...
        const node = {
          ...
          y: Math.random(),
          mid: function () {
            const dx = this.width / 2.0 + this.x,
  ...
  function drawNodeBars(graph) {
    ...
    for (let i = 0; i < graph.nodes.length; i++) {
      ...
      for (let j = 0; j < node.values.length; j++) {
        ...
        let selector = 'rect[data-node="' + node.id + '"][data-value="' + value + '"]';
        d3.select(selector)
          .attr({
            width: prob
          });
  ...
  function drawNodes(options) {
    ...
    const nodes = d3.select(options.id)
      ...
      .attr({
        ...
        class: 'node-group' + options.id
      })
      .on('mousedown', function (d) {
        d3.selectAll('g.node-group' + options.id).sort(function (a, b) {
          if (a.id !== d.id) {
            return -1;
          } else {
            return 1;
    ...
    const text_tag = nodes.append('text')
      .attr({
        x: function (d) {
          return d.width / 2;
        },
  ...
  function drawEdges(options) {
    const graph = options.graph;
    const edges = d3.select(options.id)
      .selectAll('line')
      .data(graph.edges)
      .enter()
      .append('line')
      .each(function (d) {
        ...
        d3.select(this).attr({
          ...
          class: 'edge-line',
          'marker-end': 'url(#arrow' + options.id + ')'
        });
      });
  }
  ...
  function defineLib() {
    ...
    lib.fromGraph = function (graph, svg_id) {
      let g = newGraph();

      for (let i = 0; i < graph.nodes.length; i++) {
        let n = graph.nodes[i];
        g.addNode(n.name, svg_id, n.name, n.values, normalize(n.sampledLw));
        for (let j = 0; j < n.parents.length; j++) {
          let p = n.parents[j];
          g.addEdge(p.name.toString() + svg_id.toString(),
            n.name.toString() + svg_id.toString());
        }
      }
